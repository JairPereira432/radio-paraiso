...
<div class="max-w-4xl mx-auto px-4 py-10">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            ...
            <h1 class="text-3xl font-black text-gray-800">✏️ Editar Noticia</h1>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.news.update', $news) }}" enctype="multipart/form-data"
        ...
        <div>
            ...
            <input name="title" type="text" required value="{{ old('title',$news->title) }}" class="input-rp">
        </div>
        <div>
            <label class="block text-sm font-bold text-gray-600 mb-1.5">Extracto</label>
            <input name="excerpt" type="text" value="{{ old('excerpt',$news->excerpt) }}" class="input-rp">
        </div>
        <div>
            <label class="block text-sm font-bold text-gray-600 mb-1.5">Contenido *</label>
            <textarea name="body" rows="12" required class="input-rp resize-none">{{ old('body',$news->body) }}</textarea>
        </div>
        <div class="grid md:grid-cols-2 gap-5">
            <div>
                <label class="block text-sm font-bold text-gray-600 mb-1.5">Categoría</label>
                <input name="category" type="text" value="{{ old('category',$news->category) }}" class="input-rp">
            </div>
            <div>
        ...
        <div class="flex items-center gap-3">
            <input name="featured" type="checkbox" id="featured" value="1"
                   class="accent-teal-400 w-4 h-4" {{ old('featured',$news->featured) ? 'checked':'' }}>
        ...
        <div>
            ...
            @if($news->image)
            <img src="{{ Storage::url($news->image) }}" class="w-32 h-20 object-cover rounded-xl mb-3">
            <p class="text-xs text-gray-400 mb-2">Sube una nueva para reemplazarla</p>
            @endif
